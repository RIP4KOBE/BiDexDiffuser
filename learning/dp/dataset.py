...
def sample_sequence(
    ...
    for key, input_arr in train_data.items():
        ...
        if (sample_start_idx > 0) or (sample_end_idx < sequence_length):
            ...
            # Apply Padding and Insert Sequence
...
def get_data_stats(data):
    ...
    stats = {"min": np.min(data, axis=0), "max": np.max(data, axis=0)} # Calculate Statistics
    if np.any(stats["max"] > 1e5) or np.any(stats["min"] < -1e5): # Check for Outliers
...
class Dataset(torch.utils.data.Dataset):
    def __init__(
        ...
        # <editor-fold desc="Dataset Initialization">
        ...
        # <editor-fold desc="Create Sample Indices and Normalize training data">
        ...
        if stats is None:
            ...
            for key, data in train_data.items():
                stats[key] = get_data_stats(data) # computes statistics, such as minimum and maximum values, which are essential for data normalization
    ...
    def __getitem__(self, idx):
        # <editor-fold desc="Fetch Index Information">
        ...
        # <editor-fold desc="Retrieve and Normalize Data">
        ...
        # <editor-fold desc="Process Representation-Specific Data">
